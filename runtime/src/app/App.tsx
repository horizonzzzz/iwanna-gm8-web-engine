  ...
  const hudCards = useMemo(
    () => [
      ...
      },
      {
        id: "runtime-player",
        label: "Player",
        value: shell.snapshot?.player
          ? `Player: x=${shell.snapshot.player.x}`
          : "Player: unavailable",
      },
      {
        id: "runtime-input",
      ...
      },
      {
        ...
        label: "Events",
        value:
          shell.snapshot?.diagnostics
            .filter((item) => item.includes("runtime-"))
            .slice(-1)[0] ?? "Events: none",
      },
      {
        id: "runtime-diagnostics",
        label: "Diagnostics",
        value: shell.snapshot
          ? `Diagnostics: ${Math.min(shell.snapshot.diagnostics.length, 8)} recent`
          : "Diagnostics: none",
      },
      {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <ControlBar
        ...
        onPackageSelect={(path) =>
          void shell.loadCurrentPackage(keyboardRef, path)
        }
      ...
      <section className="mx-auto flex w-full max-w-[1800px] flex-col gap-4 px-6 py-6">
        ...
        <DebugReportPanel
          title="Debug Report"
          report={debugReport}
          collisionOverlayEnabled={shell.collisionOverlayEnabled}
          onCollisionOverlayChange={shell.setCollisionOverlayEnabled}
        />
