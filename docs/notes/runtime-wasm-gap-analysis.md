...
- The same `runtime-diagnostics` command can now collect headless player behavior traces without adding a separate CLI tool. Use `--trace-player --trace-every <n>` to add a sampled `player_trace` array to the diagnostics JSON, or add `--trace-output <path>` for longer runs. Each trace row records room/tick, player object and runtime id, position, velocity, alive state, grounded/jump phase state, active input trace, and diagnostic count. This is the first command-line behavior-validation surface for comparing Dife movement and jump trajectories after unsupported runtime blockers are empty.
