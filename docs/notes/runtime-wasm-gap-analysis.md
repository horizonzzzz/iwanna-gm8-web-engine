...
- Runtime-core collision event dispatch now indexes live instances by `object_id` before checking collision targets, expands collision targets through child objects that inherit from the declared target, and still dispatches the original parent-target event block. Collision events also roll solid participants back to their previous positions before running the event block, matching GM8/OpenGMK solid-collision ordering closely enough for Dife `blood2` to resolve against `block` through `move_contact_solid()`. Collision-event expression evaluation now reuses a spatial index for object-target, solid, and two-argument `place_free(x,y)` checks, so Dife room151 death blood no longer repeatedly scans the full room for each particle collision. Collision block-level trace diagnostics are suppressed on this hot path; unsupported diagnostics and non-collision execution traces remain available.
