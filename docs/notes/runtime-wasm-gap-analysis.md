...
- Runtime-core restart input now resolves runtime/package globals such as `global.restartbutton` and `global.resetbutton` before falling back to host key `R` (`82`) for explicit host restart input. Treat `R` as the current headless/host fallback reset key, not as a browser-shell IWanna rule. Runtime-core no longer automatically restarts the room on direct hazard death; reset remains an explicit input edge. If a package-owned keyboard event already requested a room transition or reset on the same tick, the fallback restart no longer stacks a second reset on top of that package request.
