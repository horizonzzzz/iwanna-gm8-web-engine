...
- Runtime-core collision event dispatch now indexes live instances by `object_id` before checking collision targets, expands collision targets through child objects that inherit from the declared target, and still dispatches the original parent-target event block. Collision events also process solid hits before same-frame non-solid hits, re-read live solid flags while processing each hit, roll solid participants back to their previous positions before running the event block, and re-check stale hits after rollback. This matches GM8/OpenGMK solid-collision ordering closely enough for Dife `blood2` to resolve against `block` through `move_contact_solid()`, for upward solid ceiling collisions to clear `vspeed` before GM8-style speed reapply, and for simultaneous block/spike contacts to let the solid contact separate the player before a stale hazard collision event fires. Player-involved upward solid/hazard collision events now also emit `runtime-collision-trace` diagnostics with `pre`, `after-rollback`, `after-event`, `after-reapply`, and optional final rollback phases; these appear in the browser Debug Report diagnostics and make stuck-ceiling regressions inspectable from a single copied report. Collision-event expression evaluation now reuses a spatial index for object-target, solid, and two-argument `place_free(x,y)` checks, so Dife room151 death blood no longer repeatedly scans the full room for each particle collision. Collision block-level execution trace diagnostics are suppressed on this hot path; unsupported diagnostics and non-collision execution traces remain available.
