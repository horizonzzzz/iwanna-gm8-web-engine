...
Use `--input-script` when the behavior depends on more than a single one-tick press or an always-held key, and use `runtime_events` to quickly spot whether a restart, room change, death, or runtime instance churn happened on the expected frame. The current checked-in Dife room 143 input scripts live under `docs/notes/runtime-scenarios/` and cover tap jump, held jump, release cut, right movement, and shoot.
