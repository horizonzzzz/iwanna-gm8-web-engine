...
- A fresh local rebuild of the six non-L1/L2/L3 `gm8-core` packages validates all six. Default-path 600-tick diagnostics are blocker-free for Afflict the Mashikaku, 3200min, Experience, Favorite Difficulty, and Forever. Kamilia room69 now starts its Create-time BGM, its `S` action reaches room0, and a fresh Shift press in room0 follows the original `room = loadRoom` path through rooms 2 and 1 into testRoom (34). `execute_file`, `file_text_open_write`, `file_text_write_string`, and `file_text_close` remain unsupported in its `global_init` save-data chain, so persisted save/globaldata behavior is still incomplete. Rebuilt Kamilia packages preserve complete switch/case bodies as Raw statements and contain no fake `switch()` function calls.
