...
- Runtime-core expression evaluation now covers the Dife-critical pure helpers `abs()` and `string()`. A local Dife diagnostics run against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --select-room 143 --ticks 60 --press-keys 16` now ranks the next blockers as `runtime-unsupported-function:collision_line`, `runtime-unsupported-function:distance_to_object`, and `runtime-unsupported-statement:for`.
