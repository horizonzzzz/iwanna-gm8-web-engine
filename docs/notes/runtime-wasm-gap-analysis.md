...
- The runtime core now consumes a small create-time slice and a narrow `step` slice of `logic.lowered.json` for bootstrapping assignments plus direct `room_goto` / `game_restart` / assignment semantics. It dispatches Begin Step, ordinary Step, End Step, alarm, held-key, key-press, key-release, local/global left/right/middle mouse held/press/release, outside, boundary, path-end, and collision slices with parent fallback lookup for event dispatch. The browser bridge forwards canvas-scaled mouse coordinates and button edges as well as raw keyboard input.
...
- Runtime-core room-change semantics are closer to OpenGMK for the currently exercised save/load path: lowered scripts continue executing after `room_goto()` until the pending room change is applied, `game_restart()` restarts from the first room and clears runtime globals instead of reloading the current room, and alive `persistent` instances are carried across `room_goto()` transitions. When a persistent player enters a room that already has the same live player object placed in the room, runtime-core keeps the carried player as the canonical single-player instance instead of leaving both live.
...
- A fresh local rebuild of the six non-L1/L2/L3 `gm8-core` packages validates all six. Default-path 600-tick diagnostics are blocker-free for Afflict the Mashikaku, 3200min, Experience, Favorite Difficulty, and Forever. Kamilia room69 now starts its Create-time BGM and its `S` action reaches room0; it remains explicitly blocked by `execute_file`, `file_text_open_write`, `file_text_write_string`, and `file_text_close` in its `global_init` save-data chain. Rebuilt Kamilia packages preserve complete switch/case bodies as Raw statements and contain no fake `switch()` function calls.
