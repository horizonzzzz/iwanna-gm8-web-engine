...
- Runtime-core expression evaluation now covers the Dife-critical helpers `abs()`, `string()`, object-name `distance_to_object()`, and object-name `collision_line()`. `collision_line()` returns a hit instance id or GM `noone` (`-4`), and numeric truthiness now follows GM/OpenGMK behavior (`number >= 0.5`) so `noone` is false in conditionals. Lowered `for` loops now execute assignment-shaped init/step expressions, scoped iterator locals, loop bodies, and transition/reset interruption with an iteration guard. Parser lowering preserves compound assignment steps such as `for(i = 0; i <= 100; i += 1)` as executable assignment-shaped step expressions instead of non-mutating `i + =1` expressions. Local Dife diagnostics runs against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --select-room 143 --ticks 60 --press-keys 16` and `--ticks 240` currently report an empty `runtime_blockers` list. After the compound-step lowering fix, direct local diagnostics against `sampleroom03` (`room_id = 146`) and `room151` (`room_id = 151`) for 120 ticks also report empty `runtime_blockers` lists rather than per-tick `runtime-for-iteration-limit` warnings from `world` step.
