...
- Runtime-core default tests now keep parser/runtime behavior coverage on in-memory package fixtures. Local Dife sample-backed tests still exist, but they are behind the explicit `local-sample-tests` feature so stale `runtime/public/packages/sample` output does not affect ordinary `cargo test` runs.
