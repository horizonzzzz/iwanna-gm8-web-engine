...
- Runtime-core expression evaluation now covers the Dife-critical helpers `abs()`, `string()`, object-name `distance_to_object()`, and object-name `collision_line()`. `collision_line()` returns a hit instance id or GM `noone` (`-4`), and numeric truthiness now follows GM/OpenGMK behavior (`number >= 0.5`) so `noone` is false in conditionals. A local Dife diagnostics run against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --select-room 143 --ticks 60 --press-keys 16` now ranks the next blocker as `runtime-unsupported-statement:for`.
