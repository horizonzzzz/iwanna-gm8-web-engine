...
- Runtime unsupported diagnostics are now available on the lowered execution path. After adding runtime-core support for `abs()`, `string()`, `instance_number()`, `instance_place()`, object-name `distance_to_object()`, object-name `collision_line()`, lowered `for` loops, the Dife-critical GM numeric comparison semantics, non-player `hspeed/vspeed` motion, the Dife BGM-critical sound subset (`sound_play()` / `sound_loop()` / `sound_stop()` / `sound_stop_all()` / `sound_isplaying()`), and the world-step numlock pair (`keyboard_get_numlock()` / `keyboard_set_numlock(off)`), the current local run against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --preselect-ticks 2 --select-room 143 --ticks 240 --press-keys 16 --trace-player --trace-every 20` reports an empty `runtime_blockers` list. Direct local diagnostics against `sampleroom03` (`room_id = 146`) and `room151` (`room_id = 151`) for 120 ticks also report empty `runtime_blockers` lists, covering the recent BGM audio query/stop-all path and the previous `world` for-loop step regression path. A targeted Dife shoot-path check with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --preselect-ticks 2 --select-room 143 --ticks 20 --press-keys 90` reports `runtime-instance-created` for `bullet` at tick `3`, proving that `global.shotbutton = ord("Z")`, `playerShoot()`, `instance_create(x, y, bullet)`, and `sound_play(sndShoot)` all execute on the current lowered runtime path. The follow-up collision/destroy check `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --preselect-ticks 2 --select-room 143 --ticks 80 --press-keys 90` reports `runtime-instance-destroyed` for that same `bullet` from `object:2:event:3:0` at tick `7`, proving that the generated Dife bullet path moves forward, probes `block`, and self-destroys from its lowered `step` logic instead of only expiring later through `alarm[0]`.
- Behavior trace is now part of the same diagnostics command rather than a separate tool. For movement and jump validation, run a targeted room with a stable input script, for example `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --preselect-ticks 2 --select-room 143 --ticks 240 --press-keys 16 --trace-player --trace-every 1 --trace-output .\runtime-trace-room143.json`, then compare `trace_summary` first for first/last frames, coordinate range, peak absolute speeds, sample count, and room segments. Use full `player_trace` rows for player `x/y/hspeed/vspeed`, grounded state, jump active/hold/cut state, alive state, active keys, and room/tick transitions after the summary shows a drift or needs explanation. The `--preselect-ticks` warmup matters for Dife because the Web runtime normally lets default-room bootstrap logic initialize globals such as `global.jumpbutton` before entering a gameplay room. Current local trace baselines for `sampleroom01` with `--trace-every 20` are: one-tick Shift press over 240 ticks gives `x=81.0`, `min_y=482.8`, `max_y=567.3`, `max_abs_vspeed=6.7`, `sample_count=12`, and empty blockers; held Shift over 240 ticks currently produces the same sampled coordinate envelope with `jump_pressed=true` in sampled rows; held Right over 120 ticks gives `x=135.0..154.0`, `y=567.4`, `max_abs_hspeed=3.0`, `sample_count=6`, and empty blockers. Treat these as comparison baselines, not final proof of native-feel parity.
