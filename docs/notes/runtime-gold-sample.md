...
- First blocking warning or missing behavior: title/menu/select can now be reached through the original room-order path, and the Dife-critical `DeathTime` binary-file path plus room143 S-key `savePoint` path now have minimal runtime support. Full usability still depends on broader Draw-event rendering, menu logic, and general save/file API parity beyond the currently proven `file_bin_*` slice.
...
- Current local death/reset scripted baselines use `room151` (`room_id = 151`) because it has a stable rightward hazard path. `docs/notes/runtime-scenarios/dife-room151-death-right.json` with `--preselect-ticks 2 --select-room 151 --ticks 180 --trace-player --trace-every 20` reports empty blockers and a first death at room `151`, tick `50`, object `player`, `x=257`, `y=243.4`, `reason=hazard`. The same event window then shows package-owned death feedback: `bloodEmitter2` and `GAMEOVER` are created at tick `50`, the emitter Step loop creates moving red `blood2` particles before the emitter's alarm destroys it, and no room restart occurs during the 180-tick run. A longer local debug run of the same script with `--ticks 1200 --trace-output target\runtime-dife-room151-1200-final.json` reaches tick `1202`, reports `287` `blood2` creates, `0` blockers, and no collision trace diagnostics. `docs/notes/runtime-scenarios/dife-room151-death-then-reset.json` with `--ticks 100` reports the same death feedback creation, then a reset/restart at tick `73` after pressing fallback `R`, returning the player to the room151 savepoint and clearing the death objects through room reload. `docs/notes/runtime-scenarios/dife-room151-r-reset.json` with `--ticks 80` reports empty blockers, one `runtime-room-restart-requested` at tick `3`, and a room restart from `151` to `151`. The `R` key in these scripts is the current host fallback reset key; runtime-core checks `global.restartbutton` and `global.resetbutton` before using that fallback.
