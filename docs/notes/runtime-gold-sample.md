...
- Runtime unsupported diagnostics are now available on the lowered execution path. After adding runtime-core support for `abs()`, `string()`, object-name `distance_to_object()`, object-name `collision_line()`, lowered `for` loops, and host-dispatched `sound_play()` / `sound_loop()` / `sound_stop()`, the current local runs against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --select-room 143 --ticks 60 --press-keys 16` and `--ticks 240` both report an empty `runtime_blockers` list.
