...
- First blocking warning or missing behavior: title/menu/select can now be reached through the original room-order path, and the Dife-critical `DeathTime` binary-file path now has minimal runtime support. Full usability still depends on broader Draw-event rendering, menu logic, and general save/file API parity beyond the currently proven `file_bin_*` slice.
