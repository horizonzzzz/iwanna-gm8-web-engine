...
- Runtime unsupported diagnostics are now available on the lowered execution path. After adding runtime-core support for `abs()`, `string()`, object-name `distance_to_object()`, and object-name `collision_line()`, the current local run against `sampleroom01` (`room_id = 143`) with `cargo run -p iwm-cli -- runtime-diagnostics --input .\runtime\public\packages\sample --select-room 143 --ticks 60 --press-keys 16` ranks the remaining first blocker set as:
