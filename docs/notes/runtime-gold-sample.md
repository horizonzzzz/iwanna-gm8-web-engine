...
- `runtime/public/packages/gm8-core/<game name>/` is the intended local output path for generated smoke packages (one directory per game so L1/L2/L3 builds never overwrite each other), not a tracked artifact
...
This is the intended local browser smoke target after generating a package from the gold sample. Each sample builds into its own `runtime/public/packages/gm8-core/<game name>/` directory (`.\scripts\build-samples.ps1` builds the L1/L2/L3 baselines in one pass). The repo does not currently ship a checked package artifact there.

## Blockers By Layer

- Parser/package availability: a fresh clone does not include `runtime/public/packages/gm8-core/IWBT_Dife/`, so the first gold-sample smoke prerequisite is still a local `build-package` run against `IWBT_Dife`.
- Package-contract validation: after generating `runtime/public/packages/gm8-core/IWBT_Dife/`, run `cargo run -p iwm-cli -- validate-package --input .\runtime\public\packages\gm8-core\IWBT_Dife` before treating browser symptoms as runtime-semantic failures. As of the current local IWBT_Dife smoke, the generated sample validates successfully; hidden title/menu background layers no longer block validation because they are not drawn by the current renderer contract.
...
- Wasm/web host gap: the host telemetry path exists, but gold-sample-specific runtime claims still require evidence from a locally generated `gm8-core/IWBT_Dife/` package.
- Shell-only issue: the default package path is `/packages/gm8-core/IWBT_Dife`, so missing local artifacts should surface as explicit load errors rather than being mistaken for runtime-semantic failures.
...
- Intended package path: `runtime/public/packages/gm8-core/IWBT_Dife/`
...
- SavePoint feedback animation: the room143 and room147 local-sample regressions now verify that pressing `S` creates exactly seven moving `object809` bubbles whose first Step reads GM's default `image_alpha = 1` and fades to `0.98`, while `object808` is created at the live active-view origin and submits the full-view Sprite 524 at canvas `(0, 0)` with positive alpha. Stable GM depth ordering keeps the overlay in front without a Dife-specific runtime rule, and the existing `object819` alarm still recreates the savePoint afterward.
...
- Do not treat the absence of `runtime/public/packages/gm8-core/IWBT_Dife/` in a fresh clone as evidence of a runtime failure; it is a local artifact prerequisite.
